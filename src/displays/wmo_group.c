...
static GtkWidget *build_modr(struct wmo_group_display *display)
{
	...
	for (uint32_t i = 0; i < display->file->modr.doodads_nb; ++i)
	{
		GtkTreeIter iter;
		gtk_list_store_append(store, &iter);
		SET_TREE_VALUE_U64(0, i);
		SET_TREE_VALUE_U64(1, display->file->modr.doodads[i]);
	}
	gtk_tree_view_set_model(GTK_TREE_VIEW(tree), GTK_TREE_MODEL(store));
	gtk_widget_show(tree);
	return tree;
...
static GtkWidget *build_mobr(struct wmo_group_display *display)
{
	...
	ADD_TREE_COLUMN(0, "id");
	ADD_TREE_COLUMN(1, "indice");
	for (uint32_t i = 0; i < display->file->mobr.indices_nb; ++i)
	{
		GtkTreeIter iter;
		gtk_list_store_append(store, &iter);
		SET_TREE_VALUE_U64(0, i);
		SET_TREE_VALUE_U64(1, display->file->mobr.indices[i]);
	}
	gtk_tree_view_set_model(GTK_TREE_VIEW(tree), GTK_TREE_MODEL(store));
	gtk_widget_show(tree);
	return tree;
...
static GtkWidget *build_mocv(struct wmo_group_display *display)
{
	...
	ADD_TREE_COLUMN(3, "b");
	ADD_TREE_COLUMN(4, "a");
	for (uint32_t i = 0; i < display->file->mocv.colors_nb; ++i)
	{
		const struct wow_vec4b *mocv = &display->file->mocv.colors[i];
		GtkTreeIter iter;
		gtk_list_store_append(store, &iter);
		SET_TREE_VALUE_U64(0, i);
