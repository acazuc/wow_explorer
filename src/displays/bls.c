...
struct display *bls_display_new(const struct node *node, const char *path, struct wow_mpq_file *mpq_file)
{
	...
	struct bls_display *display = malloc(sizeof(*display));
	if (!display)
	{
...
static GtkWidget *build_display(struct bls_display *display, const struct wow_bls_shader *shader, bool pixel, uint32_t block)
{
	...
	gtk_paned_pack1(GTK_PANED(args), consts, true, true);
	gtk_paned_pack2(GTK_PANED(args), params, true, true);
	gtk_widget_show(args);
	/* Box */
	GtkWidget *paned = gtk_paned_new(GTK_ORIENTATION_VERTICAL);
	gtk_paned_pack1(GTK_PANED(paned), code, true, true);
	gtk_paned_pack2(GTK_PANED(paned), args, false, false);
...
static void on_gtk_block_row_activated(GtkTreeView *tree, GtkTreePath *path, GtkTreeViewColumn *column, gpointer data)
{
	...
	gtk_paned_pack2(GTK_PANED(display->display.root), build_display(display, &display->file->blocks[block].shaders[(val & 0xffff) - 1], display->file->header.header.magic == (('S' << 0) | ('P' << 8) | ('X' << 16) | ('G' << 24)), block), true, true);
}
