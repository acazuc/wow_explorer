...
struct display *blp_display_new(const struct node *node, const char *path, wow_mpq_file_t *mpq_file)
{
	...
	struct blp_display *display = malloc(sizeof(*display));
	if (!display)
	{
...
static GtkWidget *build_gtk_right_paned(struct blp_display *display)
{
	...
	gtk_container_add(GTK_CONTAINER(right_paned), display->gtk_display);
...
static GtkWidget *build_gtk_top_paned(struct blp_display *display)
{
	...
	gtk_paned_pack1(GTK_PANED(top_paned), left_paned, false, false);
	gtk_paned_pack2(GTK_PANED(top_paned), right_paned, true, false);
...
static GtkWidget *build_gtk_bot_paned(struct blp_display *display)
{
	...
	GtkWidget *bot_paned = gtk_scrolled_window_new(NULL, NULL);
	gtk_widget_set_size_request(bot_paned, 0, 200);
	gtk_container_add(GTK_CONTAINER(bot_paned), details);
...
static GtkWidget *build_gtk_paned(struct blp_display *display)
{
	...
	gtk_paned_pack1(GTK_PANED(paned), top_paned, true, true);
	gtk_paned_pack2(GTK_PANED(paned), bot_paned, false, false);
...
static void set_mipmap_box(struct blp_display *display, uint32_t mipmap_id)
{
	...
	uint32_t height;
	uint32_t line_width;
	if (!blp_decode_rgba(display->file, mipmap_id, &width, &height, &data))
		return;
	...
	gtk_box_pack_start(GTK_BOX(display->gtk_display), display->image, false, false, 0);
}
